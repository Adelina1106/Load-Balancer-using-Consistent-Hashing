...
struct info_h {
    unsigned int tag;  // eticheta replicii
    int id_server;  // id-ul serverului
    char keys[500][100];  // array ce retine cheile obiectelor
                          // salvate de fiecare replica a unui server
    unsigned int keys_size;  // cate obiecte retine fiecare eticheta
};
typedef struct info_h info_h;
